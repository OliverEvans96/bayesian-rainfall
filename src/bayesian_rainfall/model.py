...
def create_rainfall_model(data):
    ...
    is_rainy = (rainfall > 0).astype(int)
    
    # Get data for rainy days only
    rainy_mask = rainfall > 0
    rainy_rainfall = rainfall[rainy_mask]
    rainy_day_of_year = day_of_year[rainy_mask]
    
    # Cyclical encoding for seasonal pattern (all days)
    day_sin = np.sin(2 * np.pi * day_of_year / 365.25)
    day_cos = np.cos(2 * np.pi * day_of_year / 365.25)
    
    # Cyclical encoding for rainy days only
    rainy_day_sin = np.sin(2 * np.pi * rainy_day_of_year / 365.25)
    rainy_day_cos = np.cos(2 * np.pi * rainy_day_of_year / 365.25)
    ...
    with pm.Model() as model:
        ...
        rain_indicator = pm.Bernoulli("rain_indicator", p=p_rain, observed=is_rainy)
